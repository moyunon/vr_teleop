    ...
    def refresh_following_safety(self, now_monotonic):
        ...
        if engagement_start is None:
            ...
            self.safety_supervisor.update_following(
                ready=False,
                hold_required=True,
                reason="following-error engagement epoch unavailable",
            )
            return
        ...
        for side, state in self.arms.items():
            decisions[side] = self.following_monitors[side].evaluate(
                ...
                now_monotonic,
                engagement_start_monotonic=engagement_start,
            )
        ...
        awaiting_sides = [
            ...
            if not item.command_is_current_engagement
            and item.ready
            and not item.hold_required
        ]
        if hold or not ready:
            candidate_sides = [
                side
                for side, item in decisions.items()
                if item.hold_required or not item.ready
            ]
        elif awaiting_sides:
            candidate_sides = awaiting_sides
        else:
            candidate_sides = list(decisions)
        limiting_side = max(
            candidate_sides,
            key=lambda side: (
                -1.0
        ...
        if awaiting_sides and not hold and ready:
            following_reason = (
                "/".join(awaiting_sides)
                + " awaiting first post-engagement safe command"
            )
        else:
            following_reason = f"{limiting_side} {detail.reason}"
        self.safety_supervisor.update_following(
            ready=ready,
            hold_required=hold,
                ...
                data=json.dumps(
                    {
                        side: {
                            ...
                            "engagement_start_time": (
                                item.engagement_start_time
                            ),
                            "engagement_age": item.engagement_age_s,
