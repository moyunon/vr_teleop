...
void ovrVrInputStandard::RenderRunningFrame(
    ...
    for (int i = 0; i < (int)InputDevices.size(); ++i) {
        ...
        transforms.trackingHigh = trackingHigh;
        ...
        TransformMatrices[axisSurfaces++] = pointerMatrix;

        bool renderHand = trackingHigh;
        if (renderHand) {
        ...
        case ovrControllerType_TrackedRemote: {
            ...
            if (result == ovrSuccess) {
                ovrInputDeviceTrackedRemoteHand* remoteHandDevice =
                    ovrInputDeviceTrackedRemoteHand::Create(*this, remoteCapabilities, Buttons);
...
ovrInputDeviceTrackedRemoteHand* ovrInputDeviceTrackedRemoteHand::Create(
    ...
    if (result == ovrSuccess) {
        ...
        ovrInputDeviceTrackedRemoteHand* device =
            new ovrInputDeviceTrackedRemoteHand(remoteCapabilities, controllerHand, buttons);
        return device;
    } else {
...
bool ovrInputDeviceTrackedRemoteHand::Update(
    ...
    if (ret) {
        ...
        if (r == ovrSuccess) {
            ...
            const ovrHandedness controllerHand = GetHand();
